# @title Step 05 - Scenario planner builder (takes ~1.5 hrs) - writes to existing dashboard sheet
# =============================================================================
# MIGRATION NOTE: this is cell 4 of MMMv13_2.ipynb. Changes for CLI execution:
#   1. The defensive `!pip install` re-run is now an import check with a clear
#      remediation message (`colab install` runs from your LOCAL terminal, not
#      inside the kernel).
...
# tabs - is unchanged.
#
# *** PRODUCTION-FACING: this step OVERWRITES worksheets in the live dashboard
...
# ---------------------------------------------------------------------------
# Prereqs check: a runtime reset wipes installed packages AND your fitted `mmm` model.
# The import check below tells you clearly if packages are missing; the globals check
# tells you if you need to re-run step 01 (or steps/load_model.py) first.
# ---------------------------------------------------------------------------
try:
    import meridian  # noqa: F401
    import scenarioplanner  # noqa: F401
except ImportError as _e:
    raise RuntimeError(
        f"Missing package ({_e}). The runtime was likely reset. From your LOCAL terminal run: "
...
min_spend_shift_ratio = 0.40
max_spend_shift_ratio = 0.40
...
print('Running scenario inference - roughly 1.5 hrs on A100.')
...
if spreadsheet is None:
    ...
    try:
        ...
        # If the credentials object isn't directly acceptable, fall back to google.auth.default
    ...
    # the standalone spend-aligned ROAS-by-quarter step. Schema matches MediaROI exactly so Looker
...
try:
    ...
    print(f"Open the ABC MMM Scenario Planner in Looker Studio: {report_url}")
except Exception as e:
