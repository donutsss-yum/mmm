# @title Step 04 - Per-channel adstock decay profile (revenue by lag week)
# =============================================================================
# MIGRATION NOTE: this is cell 3 of MMMv13_2.ipynb, unchanged except for this
# header, a kernel-state check, and the final plot being SAVED to the Drive
# diagnostics area instead of shown inline (no display in a terminal exec).
#
# REQUIRES kernel state from step 01 (mmm, df_bq, media_channels,
# media_impression_cols, out_dir).
# Run via:  colab exec -s <session> -f steps/04_decay_profiles.py --timeout 3600
# =============================================================================
# ---------------------------------------------------------------------------
# WHY THIS EXISTS
...
if WITH_PLOT:
    ...
    plt.tight_layout()
    _dir   = out_dir if 'out_dir' in globals() else '.'
    _plot_path = os.path.join(_dir, f"decay_profile_plot_{_exported_at.strftime('%Y%m%d_%H%M%S')}.png")
    fig.savefig(_plot_path, dpi=120, bbox_inches='tight')
    plt.close(fig)
