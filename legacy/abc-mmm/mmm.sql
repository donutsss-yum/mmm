...
SELECT
  Time as time,
  CASE
    WHEN Time BETWEEN DATE '2023-04-02' AND DATE '2026-04-04' -- Add the last day of the last week of full data on refresh. Recently I've been moving the start time up as well to keep it to 156 weeks total (3y)
    THEN 'In Model'
    ELSE 'Out of Model'
  END AS Model_Dates,
  ...
  ROUND(SUM(Redemptions_Birthday_Signup), 2) AS Redemptions_Birthday_Signup,
  ROUND(SUM(Redemptions_Other), 2)           AS Redemptions_Other
