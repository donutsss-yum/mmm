...
Usage (from the repo root):
    .venv/bin/python scripts/append_inputs_to_gsheet.py "July 2026 Inputs" --dry-run
    .venv/bin/python scripts/append_inputs_to_gsheet.py "July 2026 Inputs"

The argument is a subfolder of the repo's inputs/ directory; filenames inside may carry
browser-download suffixes like "Emails (3).xlsx" - they're glob-matched. Read docs/INGEST.md
...
PROVENANCE: adapted 2026-08-05 from legacy/abc-mmm/append_inputs_to_gsheet.py (four months
of battle-testing, March-June 2026). Changes are confined to this docstring, connect()
(service-account key with ADC fallback), INPUTS_ROOT (repo inputs/), and the closing
reminder text. The per-tab handlers are verbatim - do not "improve" them without reading
the incident log in docs/INGEST.md first.
"""
import glob
import os
import sys
...
INPUTS_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'inputs')
...
def connect():
    """Service-account key when present (Ken's Mac: jelly-336@donut-426, Editor on the sheet
    since 2026-04-20), else Application Default Credentials (needs the 6-scope login from
    docs/SETUP.md section 3). A 403 on write = the sheet grant was lost - ask Ken to
    re-share, do not work around it."""
    ...
    if os.path.exists(SA_PATH):
        ...
        print(f'note: no service-account key at {SA_PATH}, using ADC')
...
def main():
    ...
    if not dry:
        verify(sh)
    print('\nDone. Next: update the History table in docs/INGEST.md, commit the input folder, '
          'and continue the refresh checklist (docs/INGEST.md section "The full monthly refresh").')
